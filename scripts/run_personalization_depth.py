...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare FedPer boundaries after convolution block 1 and 2."
    )
    ...
    parser.add_argument("--shared-blocks", nargs="+", type=int, default=[1, 2])
...
def main() -> None:
    args = parse_args()
    if not args.shared_blocks or any(depth not in (1, 2) for depth in args.shared_blocks):
        raise ValueError("--shared-blocks accepts only 1 and/or 2.")
    ...
    print(f"Running {len(args.shared_blocks)} personalization depths")
    for index, depth in enumerate(args.shared_blocks, start=1):
        ...
        result_name = (
            f"{base_name}_share{depth}conv_"
            f"{fed['num_rounds']}r_{fed['local_epochs']}e"
        )
        fed["shared_conv_blocks"] = depth
        ...
        print(
            f"[{index}/{len(args.shared_blocks)}] shared_conv_blocks={depth}, "
            f"rounds={fed['num_rounds']}, local_epochs={fed['local_epochs']}"
        )
        if args.dry_run:
            rows.append({"shared_conv_blocks": depth, "status": "dry-run"})
            continue
