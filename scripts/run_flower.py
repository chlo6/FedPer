...
def _build_wandb_config(
    ...
    return {
        ...
        "num_conv_blocks": federated.num_conv_blocks,
        ...
        "personal_layers": (
            f"remaining_{federated.num_conv_blocks - federated.shared_conv_blocks}_"
            "conv_blocks_and_classifier"
        ),
    }
...
def _build_run_name(config: ExperimentConfig, iid_mode: bool = False) -> str:
    ...
    return (
        f"{config.training.task}-fedper-{'iid-' if iid_mode else ''}"
...
def _init_wandb(
    ...
    run = wandb.init(
        ...
        + ["flower", "federated-learning", "fedper"]
...
def main(force_iid: bool = False) -> None:
    ...
    class FlowerSubjectClient(NumPyClient):
        ...
        def get_parameters(self, config: dict[str, Any]) -> list[Any]:
            model = create_federated_model(
                ...
                num_conv_blocks=self.experiment.federated.num_conv_blocks,
    ...
    def evaluate_fn(
        ...
        for partition in client_partitions:
            ...
            client_model = create_federated_model(
                ...
                num_conv_blocks=config.federated.num_conv_blocks,
    ...
    try:
        ...
        summary = {
            ...
            "shared_conv_blocks": config.federated.shared_conv_blocks,
            "local_conv_blocks": (
                config.federated.num_conv_blocks
                - config.federated.shared_conv_blocks
            ),
